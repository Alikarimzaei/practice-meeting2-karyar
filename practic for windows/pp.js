let users = [{ "id": 1, "firstName": "Tamma", "lastName": "Langer", "email": "tlanger0@a8.net", "gender": "Female", "money": "$333.43", "country": "Russia" },
...
// quastion1
let email = users.map((e) => {
    return e.email
})
////////////////////////
// quastion2
let filtercountry = users.filter((c) => {
    if (c.country == `China`) {
        return true
    } else {
        return false
...
let country = filtercountry.map((c) => {
    return { name: c.firstName + c.lastName, country: c.country }
})
///////////////////

// q3
let gender=users.filter((e)=>{
    if(e.gender==`Male`){
        return false
    }
})
...
let s = emailmouny.map((e) => {
    return e.email
})

/////////////////////
// quastion5
let g = users.filter((e) => {
    if (e.country == `France`) {
        return true
    }
})
let france = g.sort((a, b) => {
    return b - a
})
// let fra = g.map((d) => {
//     return d.money
// })
// let france = fra.sort((a, b) => {
//     return b - a
// })
...
// q8
let ff=users.reduce((sabet,ovz)=>{
    return ovz.gender+sabet
},
0
)
console.log(ff)
